...
{
    public sealed class ChaseActorUntilAttack : ITask
    {
        ...
        public async UniTask RunAsync(Actor actor, CancellationToken cancellationToken)
        {
            ...
            var actorAttack = actor.GetAbility<ActorAttack>();
            while (!actorAttack.ContainsTarget(targetActor) && !cancellationToken.IsCancellationRequested)
            {
